...
fn part_1(input: &Vec<String>) -> i64 {
    ...
    for i in 0..=4 {
        rounds.sort_by(|a, b| get_card_value_mapping(b.hands[4-i]).cmp(&get_card_value_mapping(a.hands[4 - i])))
    }
    rounds.sort_by(|a, b | b.hand_type().value().cmp(&a.hand_type().value()));
